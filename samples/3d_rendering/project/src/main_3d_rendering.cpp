...
void populateData(int size, int* ptr) {
    int ctr = 0;
...
int main(int argc, const char** argv) {
    ...
    int* input2;
    int* output;
    int size1 = 9 * 3191;   // Number of triangles = 3191. 9 coordinate points (x0,y0,z0, ..., x2,y2,z2)
    int size2 = 1 * 1;       // Angle is a 1 dimension vector
    int size3 = 255 * 255;  // frame buffer

    printf("\n===== Host-CPU preparing data ======\n\n");
    
    if((input1 = (int*)acl_aligned_malloc(size1*sizeof(input1))) == NULL) {
        perror("Failed malloc of input1 vector");
    }
    populateData(size1, input1);
    //memset(input1, 0, size*sizeof(int));

    if((input2 = (int*)acl_aligned_malloc(size2*sizeof(input2))) == NULL) {
        perror("Failed malloc of input1 vector");
    }
    populateData(size2, input2);
    //memset(input1, 0, size*sizeof(int));

    if((output = (int*)acl_aligned_malloc(size3*sizeof(output))) == NULL) {
    ...
    status = clSetKernelArg(
            ...
            2,
            sizeof(cl_mem),
            (void*)&frame_buffer); CHECK(status);
